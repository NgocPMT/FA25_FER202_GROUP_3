...
export default function FollowersList() {
    ...
    const fetchFollowers = async () => {
      try {
        ...
        const list = rawList
          .map(
            (item) =>
              item.followedBy || item.follower || item.Follower || item.user
          )
          .filter((f) => f && f.id);
  ...
  const handleToggleFollow = async (userId, isFollowing) => {
    try {
      ...
      toast.error("Toggle follow error:", err);
    }
  };
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className={`cursor-pointer text-sm border rounded-full px-3 py-1 transition ${
                    isFollowing
